...
- The combined model × reasoning menu opens at `bottom: calc(100% + 8px)` with
  ...
  the row's own `catalogVendorKey`, else the provider's `catalogProviderKey`, and
  falling back to the shared generic mark when the catalog places no vendored
  artwork for it (ADR `provider-brand-marks`). The marks are decorative,
  monochrome `currentColor` paths rendered inline, and never fetched at runtime;
  a vendored mark and the generic fallback share one size and color so the
  fallback never reads as a different class of thing. After the badges, each row
  shows its published `context window · output limit` pair through the shared
  compact token formatter; an unpublished value renders as an em dash.
