...
- Composer 模型行同时展示这一对上限——先上下文窗口，再输出上限，中间以间隔号分隔——
  使选择器与设置行读法一致，一个数字即可说明模型的规模。两个取值都以已发布记录为准；
  服务两者都未发布时该行不显示上限，未发布的那一项以破折号代替。在「高级」中按模型
  填写的覆盖值与设置行保持同一属性：选择器只报告模型的已发布值。
...
运行时、上下文检查器与设置界面解析同一个有效窗口与输出上限。每个绑定记录它的
`contextWindow` 从何而来（`contextWindowSource`），该标记同样作用于 `maxTokens`：

- `catalog` —— 该数值是 models.dev 的取值，之后 `limit.context` 或 `limit.output`
  的修正会替换它。刷新后的记录（例如 `gpt-5.6-luna`，1,050,000 tokens）不再显示为
  128k 模型；在目录还没有该 ID 时加入的行，一旦记录解析出来就不再显示通用的 8.2k
  输出上限；被 models.dev 修正的数值无需删除再重新添加即可到达该绑定。只有解析出来的
  models.dev 记录才算「已发布」：查询未命中而回退到通用形态时（例如自定义网关地址
  提供的是一个多家发布方都在列的 ID），它的 `128,000` 与 `8,192` 不算修正，
  已保存的目录快照继续生效。
- `user` —— 该数值来自单个模型「高级」里的输入（或其中的预设档位），目录永不替换它，
  包括本来就是通用种子值的 `128,000` 与 `8,192`。

在该标记出现之前写入的绑定没有来源标记，它们沿用历史规则且行为确定：已发布的
`limit.context` 只替换通用的 `128,000`，已发布的 `limit.output` 只替换通用的
`8,192`，其余保存值都视为用户显式值。未知模型仍使用保守的 128k / 8.2k 通用上限，
绝不会仅凭 ID 形态被提升。该标记在持久化记录中是可选的，因此旧版本写入的配置仍可读，
降级也不会因此出错。
...
Composer 模型行从左至右依次为：提供商图标、完整线模型 ID（有配置别名时用别名，
否则用已发布名称）、推理/视觉标记，以及「上下文窗口 · 输出上限」一对数值。提供商
分组标题在厂商名旁显示同一枚图标，因此列表长到标题滚出视野后，每一行仍能识别其
所属厂商。

图标由该行的 `catalogProviderKey` 决定，绝不依据显示名猜测：该 key 在主进程中经
由同一套已用于定位行元数据的别名映射一次性解析，因此被用户改名为同一名称的两个
提供商仍各自持有自己的图标，自定义名称也不会借用某厂商的美术资源。没有 bundled
图标的 key（包括目录无法定位的所有行）统一回退到共享通用图标，任何一行都不会
缺少身份标识。

图标随发布版本打包，并通过 CSS mask 渲染。运行时不从图标站点拉取任何资源，因此
选择器在离线状态下外观一致，也不会有外发请求泄露用户配置了哪些提供商；单色美术
资源保持统一的视觉重量并跟随主题文字颜色，而不是在列表中堆一排高饱和度的彩色
图标。见 ADR `provider-brand-marks`。
